...
namespace WebAirplus
{
    public partial class History : Page
    {
        ...
        protected static int endindex = 5;
        protected static int rowsize = 5;
        public DataSet data_set;
        protected void RenderPage()
        {
            DataTable data = data_set.Tables[0];
            DataTable data_5 = data.Copy();
            if (numrecords> rowsize)
            {
                for(int i=0;i<startindex;i++)
                {
                    data_5.Rows[i].Delete();
                }  
                for(int k = endindex- startindex; k < numrecords- startindex; k++)
                {
                    data_5.Rows[k].Delete();
                }
                GuestHolder.Controls.Clear();
            }
            DataTable statuscode_data = data_set.Tables[1];
            ...
            int leavetoprow = 0;
            foreach (DataRow dr in data_5.Rows)
            {
        ...
        {
            data_set = Datalayer.GetUserListHistory(Convert.ToString(Session["UserName"]));
            numrecords = data_set.Tables[0].Rows.Count;
        ...
        protected void btn_email_Click(object sender, EventArgs e)
        {
            ...
            DataSet dataset = Datalayer.GetUserListHistory(Convert.ToString(Session["UserName"]));
